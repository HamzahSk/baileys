...
// Memastikan kompatibilitas penuh antara Node.js dan Bun
const getRandomValues = nodeCrypto.getRandomValues || globalThis.crypto?.getRandomValues?.bind(globalThis.crypto);
const randomUUID = nodeCrypto.randomUUID || globalThis.crypto?.randomUUID?.bind(globalThis.crypto);
...
export const prepareRichResponseMessage = (content) => {
    let { code, contentText, disclaimerText, footerText, headerText, language, links, noHeading, richResponse, table, title } = content;
    let submessages = [];
